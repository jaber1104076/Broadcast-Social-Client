...
const TopPostInfo = ({ userpost }) => {
    const { image, message, postLike, _id } = userpost

    return (
        <div>
            <div className="rounded-md shadow-md  bg-gray-600 text-gray-100">
                <div className="flex items-center justify-between p-3">
                    <div className="flex items-center space-x-2">
                        ...
                        <div className="-space-y-1">
                            <h2 className="text-sm font-semibold leading-none">Abdullah Al Jaber</h2>
                            <span className="inline-block text-xs leading-none dark:text-gray-400">SomeWhere</span>
                        </div>
                    </div>
                    <Link to={`/mediaDetail/${_id}`}>
                        <button className='hover:text-gray-100 bg-gradient-to-r from-[#00B4DB] to-[#0083B0]  text-white px-5 py-3 rounded-md' title="Open options" type="button">
                            Details
                        </button>
                    </Link>
                </div>
                <img src={image} alt="" className="object-cover object-center w-full h-[400px] dark:bg-gray-500" />
                <div className="p-3">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-3">
                            <button type="button" title="Like post" className="flex items-center justify-center">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" className="w-5 h-5 fill-current active:bg-red-600">
                    ...
                    <div className="flex flex-wrap items-center pt-3 pb-1">
                        <div className="flex items-center space-x-2">
                            ...
                            <span className="text-sm">Liked by {' '}
                                <span className="font-semibold">Rafsan {' '}</span>and {' '}
                                <span className="font-semibold">{postLike} {' '}others</span>
                            </span>
                        </div>
                    </div>
                    <div className="space-y-3">
                        ...
                        <input type="text" placeholder="Add a comment..." className="w-full py-0.5 bg-transparent border-none rounded text-sm pl-5 text-gray-100 bg-white" />
                    </div>
                </div>
            </div>
        </div>
